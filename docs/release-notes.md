...
- Fix `lore repository instance list` carrying several entries for one root directory. Registering an instance retires any earlier registration recorded at the same path, so `lore repository create --force` and re-cloning into a wiped directory on a shared store leave one entry; `lore repository instance prune` also removes an entry whose directory now holds a different current instance, reported as `superseded` (`stale = 2` in the `RepositoryInstance` event); and a lost `.lore/instance` is recovered from the newest registration for the path rather than an arbitrary one
