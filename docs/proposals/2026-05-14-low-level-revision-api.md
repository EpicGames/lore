...
Appended during implementation. The body above is the approved proposal and is left as approved; where the built surface diverges, this section is authoritative. The design spec and its errata carry the reasoning.

- **Every write verb is a batch.** `add`, `delete`, `modify`, `metadata_set`, `metadata_get` and `metadata_clear` take `verb(handle, entries[])` rather than the single-item signatures sketched under "Write operations". A one-entry call is the one-entry case; there is no separate single-item entry point. It collapses N dispatches into one, removes the quadratic cost of N single adds into one directory, and is the transaction boundary that closes the check-then-act race between a verb's validation scan and its mutation. Mutating batches are all-or-nothing: every entry is validated before anything is applied.

- **`metadata_clear` was added**, a verb this proposal never named. The surface could set and read revision metadata but not remove a key, which reads as an omission rather than a deliberate exclusion.

- **`commit` takes no `branch` argument.** `lore_revision_tree_commit(handle, options)`. The branch it publishes to is the revision's own, read from the `branch` metadata key: set, it must name either the loaded revision's branch or a branch whose branch point is exactly the loaded revision; unset, it resolves to the loaded revision's branch. An initial revision on an empty handle must set the key. The commit message arrives the same way, as `metadata_set("message", …)`. Deciding the branch from the tree removes the case a `branch` argument invites, where the tip that advances and the branch the revision records disagree.

- **`commit` stamps three facts when the caller left them unset**: the branch, the `timestamp`, and `created-by` / `committed-by` from the current identity, under the same authorship rule the file-system commit applies when the active remote authenticates. Nothing is inherited from the revision the handle was loaded on.

- **The node-id contract after `commit` is resolved**, closing the question raised under "Read operations": node ids **persist**. Tree blocks are content-addressed and immutable once written, and serializing a revision does not relayout them, so a node id captured before a commit still resolves after it. The handle is left on the new revision and stays usable for further edits and commits without reloading — so the "close the handle and reload" step described under "Commit verb (Goal 6)" is optional rather than required.

- **Tip-collision behaviour is resolved**, closing the first unresolved question: `commit` fails and the caller decides. No automatic fast-forward, no rebase, no write-only mode. The failure carries the observed tip so the caller can reload without a second query. Concurrent commits are *not* serialized by the handle's in-flight machinery as "Non-Functional Considerations" assumes — they race through the pipeline, and a real compare-and-swap on the branch tip is what makes at most one publish.

- **A commit failure poisons the handle only once it has begun freezing the tree.** Before that — nothing staged, an unusable branch, a tree the validator refuses, a lost tip race — nothing has been written and the handle stays usable.

- **Two concurrent write calls targeting the same `(parent, name)` can both succeed**, leaving duplicate siblings; batching potentially-colliding work into one call is what avoids it. Accepted and documented rather than fixed, because the net is commit-time: the pre-commit validator rejects duplicate child names, so the cost is a late failure and never a corrupt published revision. This qualifies the "Risks" mitigation above, which claims the handle-side write operations reject malformed edits at call time — they do, within a call.

- **A node staged for deletion stays visible** to `list_children` and `node_info` until a commit freezes the tree, and both records carry a `staged_action` field naming the change staged on a node. Hiding a staged deletion is what would leave a caller unable to see what a commit will drop.
