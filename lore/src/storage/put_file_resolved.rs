...
async fn put_file_resolved_local(
    ...
    storage_call(
        ...
        async move |store, args| {
            ...
            crate::storage::fan_out_items!(items, "put_file_resolved", |item| {
                ...
                async move { put_file_resolved_item(store, &item, session).await }
            })
        },
    )
    .await
...
async fn put_file_resolved_item(
    ...
    PutItemOutcome::emit(
        item.id,
        resolve_put_file_resolved_item(store, item, session).await,
    )
}
