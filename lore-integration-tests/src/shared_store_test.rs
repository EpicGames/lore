    ...
    async fn get_once(
        ...
        let callback: LoreEventCallback = Some(Box::new(move |event: &LoreEvent| {
            if let LoreEvent::StorageGetData(d) = event
                && d.bytes.len > 0
            {
                // SAFETY: `d.bytes` points at a buffer the dispatcher keeps alive for the
                // duration of this callback, and `len` is its length in bytes. The slice is
                // copied before returning, so it never outlives that borrow.
                let slice =
