...
async fn stream_matches_file(
    receiver: &mut tokio::sync::mpsc::Receiver<Result<Bytes, ImmutableError>>,
    file: &lore_io::IoFile,
    file_size: u64,
) -> bool {
...
mod tests {
    ...
    fn stream_of(chunks: &[&[u8]]) -> tokio::sync::mpsc::Receiver<Result<Bytes, ImmutableError>> {
        let (sender, receiver) =
            tokio::sync::mpsc::channel::<Result<Bytes, ImmutableError>>(chunks.len().max(1));
        for chunk in chunks {
            sender
